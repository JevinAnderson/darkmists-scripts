...
function createRoom(room = {}, output) {
  const exits = Object.keys(room.exits)
    .map((key) => {
      return `<a target="__blank" class="room-exit" href="/${room.exits[key].vnum}.html">${key}</a>`;
    })
    .join("");
...
const createTOC = (area, output) =>
  new Promise((resolve) => {
    const rooms = Object.keys(area.rooms || {})
      .map(
        (vnum) =>
          `<tr class="area-room-row"><td class="area-room-column"><a target="__blank" class="area-room-anchor" href="/${vnum}.html">${area.rooms[vnum].title}</a></td></tr>`
      )
      .join("");
    const content = `
<!DOCTYPE html>
...
const generateAreaTOC = (areas, output) =>
  new Promise((resolve) => {
    const rows = Object.keys(areas)
      ...
      .map(
        (name) =>
          `<tr class="area-toc-table-row"><td class="area-toc-table-column"><a target="__blank" class="area-toc-area-anchor" href="/${areas[
            name
          ].filename
            .split(".")
