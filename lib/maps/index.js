...
<footer class="main-footer">
  <img src="/footer.png" alt="main-footer-image"/>
</footer>
`;
...
function createRoom(room = {}, output) {
  const exits = Object.keys(room.exits)
    .map((key) => {
      return `<a class="room-exit" href="/${room.exits[key].vnum}.html">${key}</a>`;
    })
    .join("");
...
const createTOC = (area, output) =>
  new Promise((resolve) => {
    const rooms = Object.keys(area.rooms || {})
      .map(
        (vnum) =>
          `<tr class="area-room-row"><td class="area-room-column"><a class="area-room-anchor" href="/${vnum}.html">${area.rooms[vnum].title}</a></td></tr>`
      )
      .join("");
    const content = `
<!DOCTYPE html>
...
<body class="area">
  ...
  ${writeFooter()}
  <script>
    window._map_filename = '${area.filename.split(".").shift()}_map.json';
  </script>
  <script src="https://ajax.googleapis.com/ajax/libs/jquery/3.5.1/jquery.min.js"></script>
  <!-- Should live on some cdn -->
  <script src="/main.js"></script>
...
const printMap = (map, output) =>
  new Promise((resolve) => {
    ...
    const filename = path.resolve(
      output,
      `${map.filename.split(".").shift()}_map.json`
    );
    console.log(`filename: ${filename}`);
...
const generateAreaTOC = (areas, output) =>
  new Promise((resolve) => {
    const rows = Object.keys(areas)
      .map(
        (name) =>
          `<tr class="area-toc-table-row"><td class="area-toc-table-column"><a class="area-toc-area-anchor" href="/${areas[
            name
          ].filename
            .split(".")
