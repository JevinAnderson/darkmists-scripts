      ...
      const point = {
        addRoom(room, x, y, z) {
          ...
          if (data[z][y][x] !== EMPTY_TOKEN) {
            const { vnum } = room;
            let existing = data[z][y][x];
            if (Array.isArray(existing)) {
              if (existing.some((x) => x.vnum === vnum)) {
                return false;
              }
              existing.push(room);
              return true;
            ...
            if (existing.vnum !== vnum) {
              data[z][y][x] = [existing, room];
              return true;
            }
      ...
      for (let z = 0; z < height; z++) {
        for (let y = 0; y < length; y++) {
          for (let x = 0; x < width; x++) {
            const existing = data[z][y][x];

            if (existing && existing !== EMPTY_TOKEN) {
              if (existing.vnum === vnum) {
